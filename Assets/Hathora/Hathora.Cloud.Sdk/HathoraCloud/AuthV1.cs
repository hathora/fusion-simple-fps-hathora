    ...
    public class AuthV1: IAuthV1
    {
        ...
        public async Task<LoginAnonymousResponse> LoginAnonymousAsync(LoginAnonymousRequest request)
        {
            ...
            if (httpCode == 200)
            {
                ...
                {                    
                    var obj = JsonConvert.DeserializeObject<PlayerTokenObject>(httpResponse.downloadHandler.text, new JsonSerializerSettings(){ NullValueHandling = NullValueHandling.Ignore, Converters = Utilities.GetDefaultJsonDeserializers() });
                    response.PlayerTokenObject = obj;
                }
                else
                {
        ...
        public async Task<LoginGoogleResponse> LoginGoogleAsync(LoginGoogleRequest request)
        {
            ...
            var serializedBody = RequestBodySerializer.Serialize(request, "GoogleIdTokenObject", "json", false, false);
        ...
        public async Task<LoginNicknameResponse> LoginNicknameAsync(LoginNicknameRequest request)
        {
            if (request == null)
            {
                request = new LoginNicknameRequest();
            }
            request.AppId ??= SDKConfiguration.AppId;
            ...
            var serializedBody = RequestBodySerializer.Serialize(request, "NicknameObject", "json", false, false);
            if (serializedBody != null)
            {
                httpRequest.uploadHandler = new UploadHandlerRaw(serializedBody.Body);
                httpRequest.SetRequestHeader("Content-Type", serializedBody.ContentType);
